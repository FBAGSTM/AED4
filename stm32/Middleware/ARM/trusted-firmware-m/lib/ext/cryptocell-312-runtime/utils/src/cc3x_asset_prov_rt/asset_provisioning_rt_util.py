...
def parse_config_file(config, log_file):
    ...
    if config.has_option(section_name, "keypwd-filename"):  # used for testing
        ...
        log_sync(
            log_file, "keypwd_filename: " + str(local_dict["keypwd_filename"]) + "\n"
        )
    else:
...
def main():
    ...
    if data_dict != None:
        ...
        if (
            (asset_size == 0)
            or (asset_size > CC_ASSET_PROV_MAX_ASSET_SIZE)
            or (asset_size % ASSET_BLOCK_SIZE != 0)
        ):
        ...
        result = DLLHandle.build_asset_blob(
            keyStr,
            key_size,
        ...
        print_and_log(
            log_file, "**** Generate asset BLOB completed successfully ****\n"
        )
