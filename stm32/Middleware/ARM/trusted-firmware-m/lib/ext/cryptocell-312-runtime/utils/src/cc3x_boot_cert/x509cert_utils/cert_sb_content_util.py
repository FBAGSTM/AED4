...
def CreateCertUtility(sysArgsList):
    try:
        ...
        print_and_log(
            log_file,
            str(datetime.now())
            + ": Content Certificate Utility started (Logging to "
            + log_fname
            + ")\n",
        )
        ...
        RecList = ImageFileAnalyzer(
            log_file,
            data_dict["images_table"],
            data_dict["load_verify_scheme"],
            data_dict["aes_ce_id"],
            DLLHandle,
            data_dict["aes_enc_key"],
            keyNonce.randStr,
            data_dict["crypto_type"],
        )

        print_and_log(log_file, "**** calling SBU_x509_AddPropHeaderExtension **** \n")
        propHeader = CertHeader(
            log_file,
            int("0xf", 16),
            CERT_TYPE_CONTENT,
            data_dict["aes_ce_id"],
        ...
        result = DLLHandle.SBU_x509_AddPropHeaderExtension(
            headerStr.encode("iso-8859-1")
        )
        if result != 0:
            print_and_log(
                log_file, "**** SBU_x509_AddPropHeaderExtension failed ****\n"
            )
            exit_main_func(log_file, config_fname, result)

        print_and_log(
            log_file, "**** calling SBU_x509_AddPubKeyNpBuffExtension **** \n"
        )
        result = DLLHandle.SBU_x509_AddPubKeyNpBuffExtension(
            str.encode(data_dict["cert_keypair"]),
            str.encode(data_dict["cert_keypair_pwd"]),
        )
        if result != 0:
            print_and_log(
                log_file, "**** SBU_x509_AddPubKeyNpBuffExtension failed ****\n"
            )
            exit_main_func(log_file, config_fname, result)
        ...
        print_and_log(
            log_file, "**** calling SBU_x509_AddContentCertBodyExtension **** \n"
        )
        result = DLLHandle.SBU_x509_AddContentCertBodyExtension(
            data_dict["nvcounter_val"],
            nonceStr.encode("iso-8859-1"),
            BinStr.encode("iso-8859-1"),
            len(RecList),
        )
        if result != 0:
            print_and_log(
                log_file, "**** SBU_x509_AddContentCertBodyExtension failed ****\n"
            )
            exit_main_func(log_file, config_fname, result)

        print_and_log(log_file, "**** calling SBU_x509_SetKeyAndSign **** \n")
        result = DLLHandle.SBU_x509_SetKeyAndSign(
            str.encode(data_dict["cert_keypair"]),
            str.encode(data_dict["cert_keypair_pwd"]),
        )
        if result != 0:
        ...
        result = DLLHandle.SBU_x509_BuildCertPkg(
            BinStrNotSigned.encode("iso-8859-1"),
            len(BinStrNotSigned),
            0,
            str.encode(data_dict["cert-pkg"]),
        )
        if result != 0:
