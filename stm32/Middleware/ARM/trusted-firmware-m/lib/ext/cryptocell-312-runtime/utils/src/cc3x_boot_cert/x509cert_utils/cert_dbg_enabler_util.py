...
def CreateCertUtility(sysArgsList):
    try:
        ...
        print_and_log(
            log_file,
            str(datetime.now())
            + ": Enabler Debug Certificate Utility started (Logging to "
            + log_fname
            + ")\n",
        )
        ...
        result = DLLHandle.SBU_x509_CreateAndSetHeader(
            int(CC_X509_CERT_TYPE_ENABLER_DBG)
        )
        if result != 0:
        ...
        propHeader = build_certificate_header(
            DEBUG_ENABLER_TOKEN,
            PrjDefines,
            LIST_OF_CONF_PARAMS,
            data_dict["rma_mode"],
        ...
        print_and_log(
            log_file, "**** calling SBU_x509_AddEnablerCertBodyExtension **** \n"
        )
        result = DLLHandle.SBU_x509_AddEnablerCertBodyExtension(
            debugStr.encode("iso-8859-1"),
            debugStr1.encode("iso-8859-1"),
            str.encode(data_dict["next_cert_pubkey"]),
        )
        if result != 0:
        ...
        if keyStr == None:
            ...
            result = DLLHandle.SBU_x509_BuildCertPkg(
                byref(keyStr), keyStrSize, 1, data_dict["cert_pkg"]
            )
        if result != 0:
