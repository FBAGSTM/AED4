...
def CreateCertUtility(sysArgsList):
    try:
        ...
        print_and_log(
            log_file,
            str(datetime.now())
            + ": Key Certificate Utility started (Logging to "
            + log_fname
            + ")\n",
        )
        ...
        propHeader = CertHeader(
            log_file, data_dict["hbk_id"], CERT_TYPE_KEY, 0, 0, 0, 0, PrjDefines
        )
        ...
        result = DLLHandle.SBU_x509_AddKeyCertBodyExtension(
            data_dict["nvcounter_val"], str.encode(data_dict["next_cert_pubkey"])
        )
        if result != 0:
            print_and_log(
                log_file, "**** SBU_x509_AddKeyCertBodyExtension failed ****\n"
            )
            exit_main_func(log_file, config_fname, result)
        ...
        result = DLLHandle.SBU_x509_BuildCertPkg(
            0, 0, 1, str.encode(data_dict["cert_pkg"])
        )
        if result != 0:
