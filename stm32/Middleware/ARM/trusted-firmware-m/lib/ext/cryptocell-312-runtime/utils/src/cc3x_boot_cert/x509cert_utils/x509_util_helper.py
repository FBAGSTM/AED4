...
def x509_cert_config_file_parser(section_name, config, local_dict, log_file):
    ...
    if config.has_option(section_name, "subject-name"):
        local_dict["subject_name"] = str.encode(
            config.get(section_name, "subject-name")
        )
    ...
    if config.has_option(section_name, "not-before"):
        ...
        log_sync(
            log_file,
            "not_before: "
            + str(local_dict["not_before"])
            + " ="
            + config.get(section_name, "not-before")
            + "\n",
        )
    else:
    ...
    if config.has_option(section_name, "not-after"):
        ...
        log_sync(
            log_file,
            "not_after: "
            + str(local_dict["not_after"])
            + " ="
            + config.get(section_name, "not-after")
            + "\n",
        )
    else:
    ...
    if config.has_option(section_name, "serial-num"):
        ...
        log_sync(
            log_file,
            "serial-num: "
            + str(local_dict["serial_num"])
            + " ="
            + config.get(section_name, "serial-num")
            + "\n",
        )
    else:
