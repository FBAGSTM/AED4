...
def CreateCertUtility(sysArgsList):
    try:
        ...
        dataToSign = build_certificate_header(
            DEBUG_ENABLER_TOKEN,
            PrjDefines,
            LIST_OF_CONF_PARAMS,
            data_dict["rma_mode"],
        ...
        enablerNPublicKey = GetRSAKeyParams(
            log_file,
            data_dict["cert_keypair"],
            data_dict["cert_keypair_pwd"],
            DLLHandle,
        )
        dataToSign = dataToSign + enablerNPublicKey.VarsToBinString()

        # add mask
        dataToSign = dataToSign + byte2string(
            struct.pack("<I", data_dict["debug_mask0"])
        )
        dataToSign = dataToSign + byte2string(
            struct.pack("<I", data_dict["debug_mask1"])
        )
        dataToSign = dataToSign + byte2string(
            struct.pack("<I", data_dict["debug_mask2"])
        )
        dataToSign = dataToSign + byte2string(
            struct.pack("<I", data_dict["debug_mask3"])
        )

        # add lock
        dataToSign = dataToSign + byte2string(
            struct.pack("<I", data_dict["debug_lock0"])
        )
        dataToSign = dataToSign + byte2string(
            struct.pack("<I", data_dict["debug_lock1"])
        )
        dataToSign = dataToSign + byte2string(
            struct.pack("<I", data_dict["debug_lock2"])
        )
        dataToSign = dataToSign + byte2string(
            struct.pack("<I", data_dict["debug_lock3"])
        )
