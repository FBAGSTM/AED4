...
def CreateCertUtility(sysArgsList):
    try:
        ...
        print_and_log(
            log_file,
            str(datetime.now())
            + ": Developer Debug Certificate Utility started (Logging to "
            + log_fname
            + ")\n",
        )
        ...
        enablerCertBin, enablerCertBin_size = parse_bin_file(
            data_dict["enabler_cert_pkg"]
        )

        # print_and_log(log_file, "**** calling SBU_x509_CreateAndSetHeader **** \n")
        result = DLLHandle.SBU_x509_CreateAndSetHeader(
            int(CC_X509_CERT_TYPE_DEVELOPER_DBG), 16
        )
        if result != 0:
        ...
        propHeader = build_certificate_header(
            DEBUG_DEVELOPER_TOKEN, PrjDefines, LIST_OF_CONF_PARAMS, 0, 0, 0
        )
        ...
        result = DLLHandle.SBU_x509_AddDeveloperCertBodyExtension(
            debugStr.encode("iso-8859-1"), byref(socid_buff)
        )
        if result != 0:
        ...
        result = DLLHandle.SBU_x509_BuildCertPkg(
            byref(enablerCertBin),
            enablerCertBin_size,
            1,
            str.encode(data_dict["cert_pkg"]),
        )
        if result != 0:
