...
def usage():
    print(
        """Syntax: ./hbk_gen_util.py -key <RSA public-key file name> -endian <optional endianness> -hash_format <optional hash_format> \n
...
def main():
    ...
    pubKeyFileName, endianness, hash_format, hash_file, zeros_file = (
        parse_shell_arguments()
    )
    log_file = create_log_file("gen_hbk_log.log")
    print_and_log(
        log_file,
        str(datetime.now())
        + ": generate HBK Utility started (Logging to gen_hbk_log.log)\n",
    )
    ...
    result = DLLHandle.SBU_x509_GetHashOfNAndNpFromPubKey(
        str.encode(pubKeyFileName), byref(publicKeyHash), outputSize
    )
    if result != 0:
        print_and_log(
            log_file, "unable to calc hash for " + str.encode(pubKeyFileName) + "\n"
        )
        sys.exit(1)
    ...
    WriteOutputToFile(
        HASHWordsList,
        str(hex(CalculateZeroBitCountOnData(HASHWordsList, outputWordSize))),
        outputWordSize,
        hash_file,
        zeros_file,
    )
