...
def parse_config_file(config, log_file):
    ...
    if config.has_option(section_name, "keypwd-filename"):  # used for testing
        local_dict["keypwd_filename"] = str.encode(
            config.get(section_name, "keypwd-filename")
        )
        log_sync(
            log_file, "keypwd-filename: " + str(local_dict["keypwd_filename"]) + "\n"
        )
    else:
...
def main():
    ...
    print_and_log(
        log_file,
        str(datetime.now())
        + ": Asset provisioning Utility started (Logging to "
        + log_fname
        + ")\n",
    )
    ...
    if data_dict != None:
        ...
        userData_size, userData_Str = GetDataFromBinFile(
            log_file, data_dict["unique_data"]
        )
        ...
        result = DLLHandle.build_asset_pkg(
            keyStr,
            key_size,
        ...
        print_and_log(
            log_file, "**** Generate asset package completed successfully ****\n"
        )
