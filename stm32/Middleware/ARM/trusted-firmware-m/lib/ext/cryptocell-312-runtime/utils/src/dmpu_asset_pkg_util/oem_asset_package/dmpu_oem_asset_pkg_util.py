...
def parse_config_file(config, log_file):
    ...
    local_dict["oem_enc_keypair"] = str.encode(
        config.get(section_name, "oem-enc-keypair")
    )
    ...
    if config.has_option(section_name, "oem-enc-keypwd"):  # used for testing
        local_dict["oem_enc_keypwd"] = str.encode(
            config.get(section_name, "oem-enc-keypwd")
        )
        log_sync(
            log_file, "oem-enc-keypwd: " + str(local_dict["oem_enc_keypwd"]) + "\n"
        )
    else:
...
def main():
    ...
    print_and_log(
        log_file,
        str(datetime.now())
        + ": OEM Asset provisioning Utility started (Logging to "
        + log_fname
        + ")\n",
    )
    ...
    if data_dict != None:
        ...
        encKey_size, encKey_Str = GetDataFromBinFile(
            log_file, data_dict["icv_enc_oem_key"]
        )
        ...
        result = DLLHandle.build_oem_asset_pkg(
            data_dict["oem_enc_keypair"],
            data_dict["oem_enc_keypwd"],
            encKey_Str,
            encKey_size,
        ...
        print_and_log(
            log_file, "**** Generate OEM asset package completed successfully ****\n"
        )
