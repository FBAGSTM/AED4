...
def key_cert_config_file_parser(config_fname, log_file):
    ...
    try:
        ...
        print_and_log(
            log_file, "Failed opening " + config_fname + " (" + e.strerror + ")\n"
        )
        sys.exit(e.errno)
    ...
    if not config.has_section(section_name):
        print_and_log(
            log_file, "section " + section_name + " wasn't found in cfg file\n"
        )
        return None, None
    ...
    if (
        local_dict["hbk_id"] != int(0)
        and local_dict["hbk_id"] != int(1)
        and local_dict["hbk_id"] != int(2)
    ):
...
def content_cert_config_file_parser(config_fname, log_file):
    ...
    local_dict["load_verify_scheme"] = int(
        config.get(section_name, "load-verify-scheme")
    )
    if (
        local_dict["load_verify_scheme"] != int(0)
        and local_dict["load_verify_scheme"] != int(1)
        and local_dict["load_verify_scheme"] != int(2)
        and local_dict["load_verify_scheme"] != int(3)
    ):
        log_sync(log_file, "Illegal load_verify_scheme defined - exiting\n")
        return None, None
    log_sync(
        log_file, "load-verify-scheme: " + str(local_dict["load_verify_scheme"]) + "\n"
    )

    local_dict["aes_ce_id"] = int(config.get(section_name, "aes-ce-id"))
    if (
        local_dict["aes_ce_id"] != int(0)
        and local_dict["aes_ce_id"] != int(1)
        and local_dict["aes_ce_id"] != int(2)
    ):
...
def enabler_cert_config_file_parser(config_fname, log_file):
    ...
    if config.has_option(section_name, "cert-keypair-pwd"):  # used for testing
        local_dict["cert_keypair_pwd"] = config.get(section_name, "cert-keypair-pwd")
        log_sync(
            log_file, "cert-keypair-pwd: " + str(local_dict["cert_keypair_pwd"]) + "\n"
        )
    else:
        local_dict["cert_keypair_pwd"] = ""
    ...
    local_dict["next_cert_pubkey"] = config.get(section_name, "next-cert-pubkey")
    log_sync(
        log_file, "next-cert-pubkey: " + str(local_dict["next_cert_pubkey"]) + "\n"
    )
...
def developer_cert_config_file_parser(config_fname, log_file):
    ...
    log_sync(
        log_file, "enabler-cert-pkg: " + str(local_dict["enabler_cert_pkg"]) + "\n"
    )
    local_dict["cert_pkg"] = config.get(section_name, "cert-pkg")
    log_sync(log_file, "cert-pkg: " + str(local_dict["cert_pkg"]) + "\n")
...
def build_certificate_header(
    token, PrjDefines, LIST_OF_CONF_PARAMS, isRmaCert, hbkId, lcsId
):

    certVersion = (
        (PrjDefines[LIST_OF_CONF_PARAMS.index("CERT_VERSION_MAJOR")]) << 16
    ) | (PrjDefines[LIST_OF_CONF_PARAMS.index("CERT_VERSION_MINOR")])
    certLength = HEADER_SIZE_IN_BYTES  # token + version + length + flags
    certLength = (
        certLength + PUBKEY_SIZE_BYTES + NP_SIZE_IN_BYTES
    )  # certificate public key + Np
    ...
    if token == DEBUG_ENABLER_TOKEN:
        certLength = (
            ...
            + 4 * BYTES_WITHIN_WORD
            + 4 * BYTES_WITHIN_WORD
            + SHA_256_HASH_SIZE_IN_BYTES
        )  # debug mask + debug lock + developer pubKey hash
        flag = build_flag_word_enabler(isRmaCert, hbkId, lcsId)
    else:
        certLength = (
            certLength + 4 * BYTES_WITHIN_WORD + SOC_ID_SIZE_IN_BYTES
        )  # debug mask + socId
    ...
    headerStrBin = (
        struct.pack("<I", token)
        + struct.pack("<I", certVersion)
        + struct.pack("<I", certLength)
        + struct.pack("<I", flag)
    )
