...
def parse_config_file(config, log_file):
    ...
    log_sync(
        log_file, "oem-main-keypair: " + str(local_dict["main_keypair_filename"]) + "\n"
    )
    ...
    if config.has_option(section_name, "oem-main-keypwd"):  # used for testing
        ...
        log_sync(
            log_file,
            "oem-main-keypwd: " + str(local_dict["main_keypwd_filename"]) + "\n",
        )
    else:
    ...
    log_sync(
        log_file, "oem-enc-pubkey: " + str(local_dict["enc_pubkey_filename"]) + "\n"
    )

    local_dict["cert_pkg_filename"] = str.encode(
        config.get(section_name, "oem-cert-pkg")
    )
...
def main():
    ...
    print_and_log(
        log_file,
        str(datetime.now())
        + ": OEM key request Utility started (Logging to "
        + log_fname
        + ")\n",
    )
    ...
    if data_dict != None:
        ...
        certLength = (
            DMPU_CERT_HEADER_SIZE_IN_BYTES
            + PUBKEY_SIZE_BYTES
            + NP_SIZE_IN_BYTES
            + PUBKEY_SIZE_BYTES
            + NP_SIZE_IN_BYTES
        ...
        mainNPublicKey = GetRSAKeyParams(
            log_file,
            data_dict["main_keypair_filename"],
            data_dict["main_keypwd_filename"],
            DLLHandle,
        )
        dataToSign = dataToSign + mainNPublicKey.VarsToBinString()

        # get encrypted  public key  (from public key)
        encNPublicKey = GetRSAPubKeyParams(
            log_file, data_dict["enc_pubkey_filename"], DLLHandle
        )
        dataToSign = dataToSign + encNPublicKey.VarsToBinString()

        # Do RSA signature
        Signature = GetRSASignature(
            log_file,
            dataToSign,
            data_dict["main_keypair_filename"],
            data_dict["main_keypwd_filename"],
