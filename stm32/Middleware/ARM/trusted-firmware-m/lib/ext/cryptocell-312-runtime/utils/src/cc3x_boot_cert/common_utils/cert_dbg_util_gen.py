...
def GetRSAKeyParams(logFile, RSAKeyFileName, PassphraseFileName, SBU_Crypto):
    ...
    result = SBU_Crypto.SBU_GetNAndNpFromKeyPair(
        str.encode(RSAKeyFileName), str.encode(PassphraseFileName), publicKey
    )
    if result != 0:
...
def GetPubKeyHash(logFile, RSAPubKeyFileName, CryptoDLL_handle):
    ...
    result = CryptoDLL_handle.SBU_GetHashOfNAndNpFromPubKey(
        str.encode(RSAPubKeyFileName), hashData, SHA_256_HASH_SIZE_IN_BYTES
    )
    if result != 0:
