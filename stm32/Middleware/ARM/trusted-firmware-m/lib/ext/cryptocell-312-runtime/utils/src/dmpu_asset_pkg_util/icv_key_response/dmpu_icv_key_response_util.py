...
def main():
    ...
    print_and_log(
        log_file,
        str(datetime.now())
        + ": ICV key response Utility started (Logging to "
        + log_fname
        + ")\n",
    )
    ...
    if data_dict != None:
        ...
        if cert_size != OEM_KEY_REQ_CERT_SIZE:
            print_and_log(
                log_file, "invalid certificate size " + str(cert_size) + " \n"
            )
        ...
        result = DLLHandle.generateIcvKeyRespPkg(
            keyStr,
            key_size,
        ...
        print_and_log(
            log_file, "**** ICV key response utility completed successfully ****\n"
        )
