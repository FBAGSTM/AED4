...
def CreateCertUtility(sysArgsList):
    try:
        ...
        CertDataList.append(
            CertHeader(
                log_file, ArgsDict["hbk_id"], CERT_TYPE_KEY, 0, 0, 0, 0, PrjDefines
            )
        )
        ...
        RSAPubKey = GetRSAKeyParams(
            log_file, ArgsDict["cert_keypair"], ArgsDict["cert_keypair_pwd"], DLLHandle
        )
        ...
        CertDataList.append(
            GetPubKeyHash(log_file, ArgsDict["next_cert_pubkey"], DLLHandle)
        )
        ...
        Signature = GetRSASignature(
            log_file,
            BinStr,
            ArgsDict["cert_keypair"],
            ArgsDict["cert_keypair_pwd"],
            DLLHandle,
        )
