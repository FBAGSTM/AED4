...
class CertHeader:
    ...
    def __init__(
        ...
        self.CertHeaderFlagCreate(
            log, hbkId, codeEncId, loadVerifyScheme, cryptoType, numOfComps
        )
        self.CertHeaderLenCreate(log, numOfComps)
        self.CertVersion = (
            (self.PrjDefines[LIST_OF_CONF_PARAMS.index("CERT_VERSION_MAJOR")]) << 16
        ) | (self.PrjDefines[LIST_OF_CONF_PARAMS.index("CERT_VERSION_MINOR")])
    ...
    def CertHeaderFlagCreate(
        self, log, hbkId, codeEncId, loadVerifyScheme, cryptoType, numOfComps
    ):
        flag = 0
    ...
    def CertHeaderLenCreate(self, log, numOfComps):
        ...
        if numOfComps > 0:  # this is a content certificate
            self.CertSize = (
                ...
                + numOfComps
                * (
                    HASH_ALGORITHM_SHA256_SIZE_IN_WORDS
                    + SW_REC_ADDR32_SIGNED_DATA_SIZE_IN_WORDS
                )
            )
