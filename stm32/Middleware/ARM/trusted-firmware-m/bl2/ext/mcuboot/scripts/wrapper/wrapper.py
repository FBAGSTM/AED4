...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="Pad image to the size determined by --layout, adding " "trailer magic",
)
@click.option(
    "-l", "--layout", help="The file containing the macros of the " "slot sizes"
)
@click.option(
    "--pad-header",
    default=False,
...
@click.option(
    "--align", type=click.Choice(["1", "2", "4", "8", "16", "32"]), required=True
)
@click.option(
    "--public-key-format",
    type=click.Choice(["hash", "full"]),
    default="hash",
    help="In what format to add the public key to "
...
def wrap(
    ...
    if enckey and key:
        if isinstance(key, imgtool.keys.RSA) and not isinstance(
            enckey, imgtool.keys.RSAPublic
        ):
            # FIXME
    ...
    img.create(
        ...
        boot_record,
        None,
        encrypt_keylen=int(encrypt_keylen),
    )
