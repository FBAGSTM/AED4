...
def generate_device_cert(
    ...
    if True:
        ...
        dev_pub_key = crypto.load_publickey(
            crypto.FILETYPE_PEM, open(dev_pub_key_path, "r").read()
        )
    else:
        # for development only, use public key from self generated private key
        dev_priv_key = crypto.load_privatekey(
            crypto.FILETYPE_ASN1, open("keys/device_priv_key.der", "rb").read()
        )
        dev_pub_key = crypto.load_publickey(
            crypto.FILETYPE_PEM,
            crypto.dump_publickey(crypto.FILETYPE_PEM, dev_priv_key),
        )
    ca_privatekey = crypto.load_privatekey(
        crypto.FILETYPE_PEM, open(ca_priv_key_path, "r").read()
    )
    ca_cert = crypto.load_certificate(
        crypto.FILETYPE_PEM, open(ca_cert_path, "r").read()
    )
    ...
    open("certificates/device_cert.pem", "wb").write(
        crypto.dump_certificate(crypto.FILETYPE_PEM, cert)
    )
    open("certificates/device_cert.der", "wb").write(
...
def main(argv):
    ...
    if not options.policy_file:
        ...
        options.policy_file = os.path.join("policy", options.policy_file)
        answer = input(
            "Policy file name was not provided, use default {}? (Y/n): ".format(
                options.policy_file
            )
        )
    ...
    if not options.device:
        ...
        answer = input(
            "\r\nDevice is not provided, use default {}? (Y/n): ".format(options.device)
        )
        if answer == "N" or answer == "n":
