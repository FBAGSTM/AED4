...
if platform.system() == "Windows":
    BL2_S_IMAGE_START = (
        os.popen("findstr BL2_S_IMAGE_START:STRING= ..\CMakeCache.txt")
        .read()
        .split("=")[1]
        .rstrip()
    )
    BL2_NS_IMAGE_START = (
        os.popen("findstr BL2_NS_IMAGE_START:STRING= ..\CMakeCache.txt")
        .read()
        .split("=")[1]
        .rstrip()
    )
    ...
    BL2_S_IMAGE_START = (
        os.popen('grep "BL2_S_IMAGE_START:STRING=" ../CMakeCache.txt')
        .read()
        .split("=")[1]
        .rstrip()
    )
    BL2_NS_IMAGE_START = (
        os.popen('grep "BL2_NS_IMAGE_START:STRING=" ../CMakeCache.txt')
        .read()
        .split("=")[1]
        .rstrip()
    )
...
os.system(
    "echo loadfile tfm_s_signed.bin " + BL2_S_IMAGE_START + " >> " + FILE
)  # flash the signed secure image into target
os.system(
    "echo loadfile tfm_ns_signed.bin " + BL2_NS_IMAGE_START + " >> " + FILE
)  # flash the signed non-secure image into target
os.system("echo r >> " + FILE)  # reset the target
...
if platform.system() == "Windows":
    os.system(
        "JLink -device lpc55s69 -if swd -speed 2000 -autoconnect 1 -commanderscript "
        + FILE
    )

else:
    os.system(
        "JLinkExe -device lpc55s69 -if swd -speed 2000 -autoconnect 1 -commanderscript "
        + FILE
    )
