...
if platform.system() == "Windows":
    BL2_S_IMAGE_START = (
        os.popen("findstr BL2_S_IMAGE_START ..\config.cmake")
        .read()
        .split("set(BL2_S_IMAGE_START")[1]
        .split('"')[1]
    )
    BL2_NS_IMAGE_START = (
        os.popen("findstr BL2_NS_IMAGE_START ..\config.cmake")
        .read()
        .split("set(BL2_NS_IMAGE_START")[1]
        .split('"')[1]
    )
else:
    BL2_S_IMAGE_START = (
        os.popen('grep "BL2_S_IMAGE_START" ../config.cmake')
        .read()
        .split("set(BL2_S_IMAGE_START")[1]
        .split('"')[1]
    )
    BL2_NS_IMAGE_START = (
        os.popen('grep "BL2_NS_IMAGE_START" ../config.cmake')
        .read()
        .split("set(BL2_NS_IMAGE_START")[1]
        .split('"')[1]
    )
...
try:
    ...
    BL2_NS_IMAGE_START = (
        BL2_S_IMAGE_START + parser.defs["values"]["FLASH_S_PARTITION_SIZE"]
    )
...
os.system(
    "cmake -S . -B build -DBL2_S_IMAGE_START="
    + BL2_S_IMAGE_START
    + " -DBL2_NS_IMAGE_START="
    + BL2_NS_IMAGE_START
    + ' -DTFM_PLATFORM=nxp/lpcxpresso55s69 -DTFM_TOOLCHAIN_FILE=toolchain_GNUARM.cmake -DTFM_PROFILE=profile_medium -DCMAKE_BUILD_TYPE=Relwithdebinfo -DTFM_ISOLATION_LEVEL=2 -G"Unix Makefiles"'
)
