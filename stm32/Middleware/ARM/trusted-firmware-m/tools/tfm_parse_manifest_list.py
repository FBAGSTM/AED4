...
def process_partition_manifests(manifest_list_files, extra_manifests_list):
    ...
    for i, manifest_item in enumerate(manifest_list):
        ...
        if is_enabled in valid_disabled_conditions:
            ...
            raise Exception(
                'Invalid "conditional" attribute: "{}" for {}. '
                "Please set to one of {} or {}, case-insensitive.".format(
                    manifest_item["conditional"],
                    manifest_item["name"],
                    valid_enabled_conditions,
        ...
        # Check if partition ID is duplicated
            ...
            raise Exception(
                "PID No. {pid} has already been used!".format(pid=manifest_item["pid"])
            )
        else:
        ...
        if "extra_path" in manifest_item:
            if not os.path.isabs(manifest_path):
                ...
                manifest_path = os.path.join(
                    manifest_item["extra_path"], manifest_path
                ).replace("\\", "/")
        ...
        manifest_head_file = os.path.join(
            OUT_DIR, output_path, "psa_manifest", "{}.h".format(manifest_out_basename)
        ).replace("\\", "/")
        intermedia_file = os.path.join(
            OUT_DIR,
            output_path,
            "auto_generated",
            "intermedia_{}.c".format(manifest_out_basename),
        ).replace("\\", "/")
        load_info_file = os.path.join(
            OUT_DIR,
            output_path,
            "auto_generated",
...
def gen_per_partition_files(context):
    ...
    manifesttemplate = ENV.get_template(
        os.path.join(sys.path[0], "templates/manifestfilename.template")
    )
    memorytemplate = ENV.get_template(
        os.path.join(sys.path[0], "templates/partition_intermedia.template")
    )
    infotemplate = ENV.get_template(
        os.path.join(sys.path[0], "templates/partition_load_info.template")
    )
    ...
    for one_partition in context["partitions"]:
        ...
        partition_context["manifest_out_basename"] = one_partition[
            "manifest_out_basename"
        ]
...
def process_stateless_services(partitions, stateless_index_max_num):
    ...
    if len(collected_stateless_services) > stateless_index_max_num:
        raise Exception(
            "Stateless service numbers range exceed {number}.".format(
                number=stateless_index_max_num
            )
        )
    ...
    for service in collected_stateless_services:
        ...
        if isinstance(service_handle, int):
            if service_handle < 1 or service_handle > stateless_index_max_num:
                raise Exception(
                    "Invalid stateless_handle setting: {handle}.".format(
                        handle=service["stateless_handle"]
                    )
                )
            ...
            if reordered_stateless_services[service_handle] is not None:
                raise Exception(
                    "Duplicated stateless_handle setting: {handle}.".format(
                        handle=service["stateless_handle"]
                    )
                )
    ...
    for i in range(0, stateless_index_max_num):
        ...
        if service != None:
            ...
            service["stateless_handle_value"] = "0x{0:08x}".format(
                stateless_handle_value
            )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Parse secure partition manifest list and generate files listed by the file list",
