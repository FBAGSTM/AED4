...
validation_funcs = {
    v: globals().get("validate_{}".format(n.lower())) for v, n in const.NAMES.items()
}
...
def validate_manadatory_claims(token, keep_going=False):
    ...
    if const.SW_COMPONENTS in token:
        if not token[const.SW_COMPONENTS] and const.NO_MEASUREMENTS not in token:
            error(
                "Invalid IAT: no software measurements defined and "
                "NO_MEASUREMENTS claim is not present."
            )

        for entry_number, sw_component_entry in enumerate(token[const.SW_COMPONENTS]):
            for mand_claim in const.MANDATORY_SW_COMPONENT_CLAIMS:
                if mand_claim not in sw_component_entry:
                    msg = (
                        'Invalid IAT: missing MANDATORY claim "{}" '
                        "from sw_componentule at index {}"
                    )
                    error(msg.format(const.NAMES[mand_claim], entry_number), keep_going)
        ...
        if entry_name == "SW_COMPONENTS":
            try:
                ...
                for raw_sw_component in value:
                    decoded_component = decode_sw_component(
                        raw_sw_component, keep_going
                    )
                    token[entry_name].append(decoded_component)
            except TypeError:
                error("Invalid SW_COMPONENT value: {}".format(value), keep_going)
...
def main():
    ...
    try:
        raw_iat = extract_iat_from_cose(
            args.keyfile, args.tokenfile, args.keep_going, args.method
        )
    ...
    if args.print_iat:
        print("Token:")
        json.dump(
            recursive_bytes_to_strings(token, in_place=True), sys.stdout, indent=4
        )
        print("")
