...
def find_tfm_root(
    ...
    target_files=["license.rst", "dco.txt", "toolchain_GNUARM.cmake"],
    max_depth=5,
):
...
if cmake_env is None:
    ...
    tfm_def_doxy_output_dir = os.path.join(tfm_def_copy_dir, tfm_def_doxy_output_dir)
    ...
    tfm_def_copy_files = True if cmake_env["SPHINXCFG_COPY_FILES"] == "True" else False
    tfm_def_render_cmake = (
        True if cmake_env["SPHINXCFG_RENDER_CONF"] == "True" else False
    )

    tfm_def_build_doxygen = (
        True if cmake_env["DOXYCFG_DOXYGEN_BUILD"] == "True" else False
    )
...
try:
    vrex = re.compile(
        r"TF-Mv(?P<VER_MAJ>\d{1,2}).(?P<VER_MIN>\d{1,2}).?"
        r"(?P<VER_HOT>\d{0,2})(?P<RC>\-RC\d)?-"
        r"(?P<PATCH_NO>\d+)-(?P<GIT_HASH>[a-g0-9]+)"
    )
    tfm_def_tfm_ver_full = (
        check_output(["git", "describe", "--tags", "--long"]).decode("UTF-8").strip()
    )
