...
@click.option(
    "-p", "--password", is_flag=True, help="Prompt for password to protect key"
)
@click.option(
    "-t",
    "--type",
...
def sign(
    ...
    if enckey and key:
        if (
            isinstance(key, keys.ECDSA256P1)
...
@click.command(
    cls=AliasesGroup, context_settings=dict(help_option_names=["-h", "--help"])
)
def imgtool():
    pass
