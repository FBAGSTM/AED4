...
class Image:
    ...
    def __init__(
        ...
        if load_addr and rom_fixed:
            raise click.UsageError(
                "Can not set rom_fixed and load_addr at the same time"
            )
        ...
        self.enctlv_len = 0
        self.max_align = (
            max(DEFAULT_MAX_ALIGN, align) if max_align is None else int(max_align)
        )
    ...
    def save(self, path, hex_addr=None):
        ...
        if ext == INTEL_HEX_EXT:
            ...
            if self.pad:
                ...
                h.puts(
                    trailer_addr + (trailer_size - len(self.boot_magic)),
    ...
    def check_header(self):
        if self.header_size > 0 and not self.pad_header:
            if any(v != 0 for v in self.payload[0 : self.header_size]):
                raise click.UsageError(
                    "Header padding was not requested and "
    ...
    def check_trailer(self):
        if self.slot_size > 0:
            ...
            if padding < 0:
                msg = (
                    "Image size (0x{:x}) + trailer (0x{:x}) exceeds "
    ...
    def create(
        ...
        if sw_type is not None:
            if len(sw_type) > MAX_SW_TYPE_LENGTH:
                msg = (
                    "'{}' is too long ({} characters) for sw_type. Its "
            ...
            boot_record = create_sw_component_data(
                sw_type, image_version, "SHA256", digest, pubbytes
            )
