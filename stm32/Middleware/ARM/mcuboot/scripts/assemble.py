...
size_re = re.compile(
    r"^#define DT_FLASH_AREA_([0-9A-Z_]+)_SIZE(_0)?\s+(0x[0-9a-fA-F]+|[0-9]+)$"
)
        ...
        with open(self.output, "ab") as ofd:
            ...
            print(
                "partition {}, pos={}, offset={}".format(
                    partition, pos, self.offsets[partition]
                )
            )
            ...
            with open(source, "rb") as rfd:
                ...
                if len(ibuf) > self.sizes[partition]:
                    raise Exception(
                        "Image {} is too large for partition".format(source)
                    )
            ofd.write(ibuf)
...
def main():
    ...
    parser.add_argument(
        "-b", "--bootdir", required=True, help="Directory of built bootloader"
    )
    parser.add_argument(
        "-p", "--primary", required=True, help="Signed image file for primary image"
    )
    parser.add_argument(
        "-s", "--secondary", help="Signed image file for secondary image"
    )
    parser.add_argument(
        "-o", "--output", required=True, help="Filename to write full image to"
    )
    parser.add_argument(
        "-z", "--zephyr-base", help="Zephyr base containing the Zephyr repository"
    )
