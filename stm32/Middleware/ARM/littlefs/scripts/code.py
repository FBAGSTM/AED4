...
def main(**args):
    ...
    if not args.get("use", None):
        ...
        with open(args["use"]) as f:
            r = csv.DictReader(f)
            results = [
                (result["file"], result["function"], int(result["size"]))
                for result in r
            ]
    ...
    def print_entries(by="function"):
        ...
        if not args.get("diff"):
            ...
            for name, (old, new, diff, ratio) in sorted(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Description of where to find *.o files. May be a directory \
            or a list of paths. Defaults to %r."
        % OBJ_PATHS,
    )
    parser.add_argument(
        "-v",
    ...
    parser.add_argument(
        "-d", "--diff", help="Specify CSV file to diff code size against."
    )
    parser.add_argument(
        "-a",
        "--all",
    ...
    parser.add_argument(
        "-s", "--summary", action="store_true", help="Only show the total code size."
    )
    parser.add_argument(
        "-q",
        "--quiet",
