...
class TestCase:
    def __init__(
        self, config, filter=filter, suite=None, caseno=None, lineno=None, **_
    ):
        self.config = config
        self.filter = filter
        self.suite = suite
    ...
    def shouldtest(self, **args):
        if (
            ...
            and len(self.filter) >= 1
            and self.filter[0] != self.caseno
        ):
            return False
        elif (
            self.filter is not None
            and len(self.filter) >= 2
            and self.filter[1] != self.permno
        ):
            return False
            ...
            while True:
                for k, v in sorted(
                    self.defines.items(), key=lambda x: len(x[0]), reverse=True
                ):
                    if k in if_:
        ...
        **args
        ...
        if gdb and failure:
            ...
            if gdb == "assert":
                ...
                ncmd.extend(
                    ["-ex", "b %s:%d" % (self.suite.path, self.code_lineno), "-ex", "r"]
                )
    ...
    def test(self, persist=False, gdb=False, failure=None, **args):
        for cycles in it.count(1):
            ...
            if gdb and failure and failure.cycleno == cycles:
                return super().test(
                    gdb=gdb, persist=persist, cycles=cycles, failure=failure, **args
                )
...
class TestSuite:
    def __init__(self, path, classes=[TestCase], defines={}, filter=None, **args):
        ...
        for i, (case, lineno) in enumerate(zip(config["case"], linenos)):
            ...
            self.cases.append(
                TestCase(
                    case, filter=filter, suite=self, caseno=i + 1, lineno=lineno, **args
                )
            )
    ...
    def permute(self, **args):
        for case in self.cases:
            ...
            case.perms = []
            for i, (class_, defines) in enumerate(it.product(self.classes, expanded)):
                case.perms.append(case.permute(class_, defines, permno=i + 1, **args))
    ...
    def build(self, **args):
        ...
        with open(self.path + ".mk", "w") as mk:
            ...
            if args.get("coverage"):
                mk.write(COVERAGE_RULES.replace(4 * " ", "\t") % dict(path=self.path))
                mk.write("\n")
            ...
            for path in tfs:
                if path is None:
                    mk.write(
                        "%s: %s | %s\n"
                        % (self.path + ".test.c", self.toml, self.path + ".test.tc")
                    )
                else:
                    mk.write(
                        "%s: %s %s | %s\n"
...
def main(**args):
        ...
        if m and not args.get("verbose"):
            try:
                ...
                sys.stdout.write(
                    ...
                    "{message}\n{line}\n\n".format(
                        path=m.group(1), line=line, lineno=lineno, message=m.group(3)
                    )
                )
            except:
                pass
    ...
    print(
        "built %d test suites, %d test cases, %d permutations"
        % (
            len(suites),
    ...
    if args.get("coverage"):
        ...
        cmd = (
            ...
            + (
                ...
                if isinstance(args["coverage"], str)
                else []
            )
            + [
                suite.path + ".info"
    ...
    print(
        "tests passed %d/%d (%.2f%%)"
        % (passed, total, 100 * (passed / total if total else 1.0))
    )
    print(
        "tests failed %d/%d (%.2f%%)"
        % (failed, total, 100 * (failed / total if total else 1.0))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help='Description of test(s) to run. By default, this is all tests \
            ...
            "test_dirs#1" or "{0}/test_dirs.toml#1#1".'.format(
            TEST_PATHS
        ),
    )
    parser.add_argument(
        "-D", action="append", default=[], help="Overriding parameter definitions."
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        "-n", "--normal", action="store_true", help="Run tests normally."
    )
    parser.add_argument(
        "-r",
        "--reentrant",
    ...
    parser.add_argument(
        "--disk", help="Specify a file to use for persistent/reentrant tests."
    )
    parser.add_argument(
        "--coverage",
        type=lambda x: x if x else True,
        nargs="?",
        const="",
