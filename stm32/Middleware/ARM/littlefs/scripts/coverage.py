        ...
        with open(path) as f:
            for line in f:
                ...
                if m and m.group("file"):
                    ...
                    funcs.append(
                        (file, int(m.group("func_lineno")), m.group("func_name"))
                    )
                elif m and file and m.group("line"):
                    lines[(file, int(m.group("line_lineno")))] += int(
                        m.group("line_hits")
                    )
    ...
    def print_entries(by="function"):
        ...
        if not args.get("diff"):
            ...
            for name, (hits, count) in sorted(entries.items()):
                print(
                    ...
                    % (
                        ...
                        "%.1f%%" % (100 * hits / count) if count else "-",
                    )
                )
        else:
            ...
            for name, (
                ...
                diff_count,
                ratio,
            ) in sorted(diff.items(), key=lambda x: (-x[1][6], x)):
                if ratio or args.get("all"):
                    print(
                        "%-36s %11s %7s %11s %7s %11s%s"
                        % (
                            name,
                            "%d/%d" % (old_hits, old_count) if old_count else "-",
                            (
                                "%.1f%%" % (100 * old_hits / old_count)
                                if old_count
                                else "-"
                            ),
                            "%d/%d" % (new_hits, new_count) if new_count else "-",
                            (
    ...
    def print_totals():
        if not args.get("diff"):
            print(
                ...
                % (
                    "TOTAL",
                    "%d/%d" % (total_hits, total_count) if total_count else "-",
                    "%.1f%%" % (100 * total_hits / total_count) if total_count else "-",
                )
            )
        else:
            ratio = (total_hits / total_count if total_count else 1.0) - (
                prev_total_hits / prev_total_count if prev_total_count else 1.0
            )
            print(
                "%-36s %11s %7s %11s %7s %11s%s"
                % (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-u", "--use", help="Don't do any work, instead use this CSV file."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--files", action="store_true", help="Show file-level coverage."
    )
    parser.add_argument(
        "-s", "--summary", action="store_true", help="Only show the total coverage."
    )
    parser.add_argument(
        "-q",
        "--quiet",
