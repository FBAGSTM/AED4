...
def chain_qd(
    ...
    if quantization_ds:
        ...
        quantized_model_path = quantize(
            cfg=cfg, quantization_ds=quantization_ds, float_model_path=float_model_path
        )
    elif train_ds:
        print(
        ...
        quantized_model_path = quantize(
            cfg=cfg, quantization_ds=train_ds, float_model_path=float_model_path
        )
    else:
        print(
            "[INFO] : Neither quantization dataset nor training set are provided! Using fake data to quantize the model. "
...
def chain_eqeb(
    ...
    valid_clip_labels: np.ndarray = None,
    quantization_ds: tf.data.Dataset = None,
    ...
    if test_ds:
        evaluate(
            ...
            model_path_to_evaluate=float_model_path,
    ...
    if quantization_ds:
        ...
        print(
            "[INFO] : No quantization or training dataset provided. Quantizing using fake data"
        )
        quantized_model_path = quantize(
            cfg=cfg,
    ...
    if test_ds:
        evaluate(
            ...
            clip_labels=test_clip_labels,
            multi_label=multi_label,
            model_path_to_evaluate=quantized_model_path,
            batch_size=batch_size,
            name_ds="test_set",
        )
    else:
    ...
    benchmark(
        cfg=cfg,
        model_path_to_benchmark=quantized_model_path,
...
def chain_tbqeb(
    ...
    benchmark(
        cfg=cfg,
        model_path_to_benchmark=trained_model_path,
        credentials=credentials,
        custom_objects=AED_CUSTOM_OBJECTS,
    )
    print("[INFO] : benchmarking complete.")
    ...
    if quantization_ds:
        ...
        quantized_model_path = quantize(
            cfg=cfg, quantization_ds=train_ds, float_model_path=trained_model_path
        )
    print("[INFO] : Quantization complete.")
...
def process_mode(
    ...
    if mode == "training":
        ...
        report_dict = None
        if test_ds:
            report_dict = evaluate(
                cfg=configs,
                eval_ds=test_ds,
                clip_labels=test_clip_labels,
                multi_label=multi_label,
                model_path_to_evaluate=None,
                batch_size=batch_size,
                name_ds="test_set",
            )
        else:
        ...
        with load_run(
            ...
            run.log_metric(
                name="patch_acc",
                value=report_dict["multiclass_classification_metrics"]["patch_acc"][
                    "value"
                ],
            )
            run.log_metric(
                name="clip_acc",
                value=report_dict["multiclass_classification_metrics"]["clip_acc"][
                    "value"
                ],
            )
        display_figures(configs)
    ...
    if get_active_config_file() is not None:
        print("[INFO] : ClearML task connection")
...
def main(cfg: DictConfig) -> None:
    ...
    if "general" in cfg and cfg.general:
        ...
        if "gpu_memory_limit" in cfg.general and cfg.general.gpu_memory_limit:
            ...
            print(
                f"[INFO] : Setting upper limit of usable GPU memory to {int(cfg.general.gpu_memory_limit)}GBytes."
            )
        else:
            print(
    ...
    # Checks if there's a valid ClearML configuration file
    print("[INFO] : ClearML config check")
    if get_active_config_file() is not None:
        print("[INFO] : ClearML initialization and configuration")
        # ClearML - Initializing ClearML's Task object.
        task = Task.init(
            project_name=cfg.general.project_name, task_name="aed_modelzoo_task"
        )
    ...
    if mode in valid_modes:
        ...
        if mode in ["chain_eqe", "chain_qb", "chain_eqeb", "chain_qd"]:
            if (
                cfg.dataset.training_audio_path
                or cfg.dataset.quantization_audio_path
                or cfg.dataset.validation_audio_path
                or cfg.dataset.test_audio_path
            ):
                (
                    train_ds,
                    valid_ds,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to folder containing configuration file",
    )
    parser.add_argument(
        "--config-name",
        type=str,
    ...
    parser.add_argument(
        "params", nargs="*", help="List of parameters to over-ride in config.yaml"
    )
