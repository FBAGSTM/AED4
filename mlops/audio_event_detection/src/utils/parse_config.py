...
from common.utils import (
    ...
    parse_tools_section,
    parse_benchmarking_section,
    parse_mlflow_section,
    parse_top_level,
    parse_general_section,
    parse_training_section,
    parse_quantization_section,
    parse_prediction_section,
    parse_deployment_section,
    check_hardware_type,
...
def _parse_dataset_section(
    cfg: DictConfig, mode: str = None, mode_groups: DictConfig = None
) -> None:
    ...
    legal = [
        ...
        "test_audio_path",
        "test_csv_path",
        "quantization_audio_path",
        "quantization_csv_path",
        "quantization_split",
    ...
    check_config_attributes(
        cfg,
        specs={"legal": legal, "all": required, "one_or_more": one_or_more},
        section="dataset",
    )
    ...
    if mode not in ["quantization", "benchmarking", "chain_qb"]:
        ...
        check_config_attributes(
            cfg,
            specs={"legal": legal, "all": None, "one_or_more": one_or_more},
            section="dataset",
        )
        if cfg.class_names:
            print("[INFO] : Using provided class names from dataset.class_names")
        elif cfg.class_names is None:
            cfg.class_names = get_class_names_from_file(cfg)
            print(
                "[INFO] : Found {} classes in label file {}".format(
                    len(cfg.class_names), cfg.classes_file_path
                )
            )
    ...
    dataset_audio_paths = [
        (cfg.training_audio_path, "training audio"),
    ...
    dataset_csv_paths = [
        (cfg.training_csv_path, "training csv file"),
...
def _parse_dataset_specific_fsd50k_section(cfg: DictConfig) -> None:
    ...
    check_config_attributes(
        cfg, specs={"legal": legal, "all": legal}, section="dataset_specific.fsd50k"
    )
...
def _parse_preprocessing_section(cfg: DictConfig) -> None:
    ...
    # All are required
    check_config_attributes(
        cfg, specs={"legal": legal, "all": legal}, section="preprocessing"
    )
...
def _parse_feature_extraction_section(cfg: DictConfig) -> None:
    ...
    # all are required
    check_config_attributes(
        cfg, specs={"legal": legal, "all": legal}, section="feature_extraction"
    )
...
def _parse_data_augmentation_section(cfg: DictConfig) -> None:
    ...
    all = ["enable"]
    check_config_attributes(
        cfg.data_augmentation.GaussianNoise,
    ...
    all = ["enable"]
    check_config_attributes(
        cfg.data_augmentation.VolumeAugment,
        specs={"legal": legal, "all": all},
...
def _get_class_names(cfg: DictConfig, name: str, csv_path=None):
    ...
    if name.lower() == "fsd50k":
        ...
        vocab_path = os.path.join(
            cfg.dataset_specific.fsd50k.csv_folder, "vocabulary.csv"
        )
        if not os.isfile(vocab_path):
            raise FileNotFoundError(
                f"Tried to infer class names for FSD50K dataset,\
...
def get_config(config_data: DictConfig) -> DefaultMunch:
    ...
    parse_top_level(
        cfg, mode_groups=mode_groups, mode_choices=mode_choices, legal=legal
    )
    ...
    if cfg.operation_mode != "benchmarking":
        ...
        _parse_dataset_section(
            cfg.dataset, mode=cfg.operation_mode, mode_groups=mode_groups
        )
    ...
    if cfg.operation_mode in mode_groups.quantization:
        ...
        parse_quantization_section(cfg.quantization, legal=legal)
    ...
    if cfg.operation_mode in mode_groups.evaluation and "evaluation" in cfg:
        ...
        parse_evaluation_section(cfg.evaluation, legal=legal)
    ...
    if cfg.operation_mode in (mode_groups.benchmarking + mode_groups.deployment):
        parse_tools_section(cfg.tools, cfg.operation_mode, cfg.hardware_type)
    ...
    if cfg.operation_mode in mode_groups.deployment:
        legal = [
            ...
            "hardware_setup",
            "build_conf",
            "unknown_class_threshold",
        ]
        legal_hw = ["serie", "board", "stlink_serial_number"]
        parse_deployment_section(cfg.deployment, legal=legal, legal_hw=legal_hw)
    ...
    if cds:  # All this breaks if no dataset section is present
        if not cds.class_names and cfg.operation_mode not in (
            "quantization",
            "benchmarking",
            "chain_qb",
        ):
            ...
            for path in [
                cds.training_csv_path,
                cds.validation_csv_path,
                cds.test_csv_path,
                cds.quantization_csv_path,
            ]:
                cds.class_names = _get_class_names(cfg, name=cds.name, csv_path=path)
                print(
                    f"[INFO] : Found {len(cds.class_names)} classes in dataset {path}"
                )
