...
def gen_h_user_file(config):
    ...
    with open(os.path.join(path, "ai_model_config.h"), "wt") as f:
        ...
        f.write(
            "#define CTRL_X_CUBE_AI_MODE_CLASS_NUMBER         ({}U)\n".format(n_classes)
        )
        ...
        f.write(
            "#define CTRL_X_CUBE_AI_SENSOR_ODR                ({}.0F)\n".format(
                config.preprocessing.target_rate
            )
        )
        ...
        f.write(
            "#define CTRL_X_CUBE_AI_SPECTROGRAM_NMEL          ({}U)\n".format(
                config.feature_extraction.n_mels
            )
        )
        f.write(
            "#define CTRL_X_CUBE_AI_SPECTROGRAM_COL           ({}U)\n".format(
                config.feature_extraction.patch_length
            )
        )
        f.write(
            "#define CTRL_X_CUBE_AI_SPECTROGRAM_HOP_LENGTH    ({}U)\n".format(
                config.feature_extraction.hop_length
            )
        )
        f.write(
            "#define CTRL_X_CUBE_AI_SPECTROGRAM_NFFT          ({}U)\n".format(
                config.feature_extraction.n_fft
            )
        )
        f.write(
            "#define CTRL_X_CUBE_AI_SPECTROGRAM_WINDOW_LENGTH ({}U)\n".format(
                config.feature_extraction.window_length
            )
        )
        if (
            config.feature_extraction.norm is None
            or config.feature_extraction.norm == "None"
        ):
            ...
            raise ValueError(
                "feature_extraction.norm must either be None or 'slaney' for Getting Started"
            )
        ...
        if config.feature_extraction.htk:
            ...
            f.write(
                "#define CTRL_X_CUBE_AI_SPECTROGRAM_FORMULA       (MEL_SLANEY) //MEL_HTK\n"
            )
        f.write(
            "#define CTRL_X_CUBE_AI_SPECTROGRAM_FMIN          ({}U)\n".format(
                config.feature_extraction.fmin
            )
        )
        f.write(
            "#define CTRL_X_CUBE_AI_SPECTROGRAM_FMAX          ({}U)\n".format(
                config.feature_extraction.fmax
            )
        )
        if int(config.feature_extraction.power) == 1:
            ...
            f.write(
                "#define CTRL_X_CUBE_AI_SPECTROGRAM_TYPE          (SPECTRUM_TYPE_POWER) // ,  /*!< squared magnitude spectrum */\n"
            )
        else:
            raise ValueError(
                f"Higher powers than 2 for spectrogram not supported in Getting Started. \n \
                             Config file had feature_extraction.power = {config.feature_extraction.power}"
            )
        if config.feature_extraction.to_db:
            f.write(
                "#define CTRL_X_CUBE_AI_SPECTROGRAM_LOG_FORMULA   (LOGMELSPECTROGRAM_SCALE_DB) //LOGMELSPECTROGRAM_SCALE_LOG\n"
            )
        else:
            f.write(
        ...
        if config.feature_extraction.to_db:
            ...
            f.write(
                "#define CTRL_X_CUBE_AI_SPECTROGRAM_SILENCE_THR   (100.0F) // 0 means disabled\n"
            )
        ...
        f.write(
            "#define CTRL_X_CUBE_AI_OOD_THR                   ({}F)\n".format(
                config.deployment.unknown_class_threshold
            )
        )
        f.write(
