...
def generate_mel_LUTs(
    sr: int, n_fft: int, n_mels: int, fmax: int, fmin: int, norm: str, htk: bool
):
    """
    ...
    mel_fb = librosa.filters.mel(
        sr=sr, n_fft=n_fft, n_mels=n_mels, fmax=fmax, fmin=fmin, norm=norm, htk=htk
    )
    ...
    assert int(
        np.sum((melFilterStopIndices - melFilterStartIndices) + 1) == len(melFilterLut)
    ), "ERROR : Mismatch between n° of coeffs and start/stop indices"

    return (
        melFilterLut,
        melFilterStartIndices.astype("int"),
        melFilterStopIndices.astype("int"),
    )
...
def _generate_hann_window_LUT(window, win_length):
    ...
    if window != "hann":
        raise ValueError(
            "The only available window type for on target pre-processing is Hanning \n \
...
def _generate_LUTs_header_file(
    path,
    melFilterLut,
    ...
    with open(os.path.join(path, "user_mel_tables.h"), "wt") as f:
        ...
        if floatingPoint == "default":
            ...
            f.write(
                "extern const float32_t user_melFiltersLut[{}];\n".format(
                    len(melFilterLut)
                )
            )
        else:
            ...
            f.write(
                "extern const PREPROC_FLOAT_T user_melFiltersLut[{}];\n".format(
                    len(melFilterLut)
                )
            )
        f.write(
            "extern const uint32_t  user_melFiltersStartIndices[{}];\n".format(
                len(melFilterStartIndices)
            )
        )
        f.write(
            "extern const uint32_t  user_melFiltersStopIndices[{}];\n".format(
                len(melFilterStopIndices)
            )
        )
        f.write("#endif /* _MEL_USER_TABLES_H */\n")


def _generate_LUTs_c_file(
    path,
    melFilterLut,
    melFilterStartIndices,
    melFilterStopIndices,
    ...
    melFilterLut_str = np.array2string(
        melFilterLut,
        separator="F ,",
        formatter={
            "float": lambda x: np.format_float_scientific(x, precision=10, unique=False)
        },
        threshold=sys.maxsize,
    )
    melFilterLut_str = "{" + melFilterLut_str[1:-1] + "F}"

    melFilterStartIndices_str = np.array2string(
        melFilterStartIndices, separator=",", threshold=sys.maxsize
    )
    melFilterStopIndices_str = np.array2string(
        melFilterStopIndices, separator=",", threshold=sys.maxsize
    )
    melFilterStartIndices_str = "{" + melFilterStartIndices_str[1:-1] + "}"
    melFilterStopIndices_str = "{" + melFilterStopIndices_str[1:-1] + "}"

    hannWin_str = np.array2string(
        hannWin,
        separator="F ,",
        formatter={
            "float": lambda x: np.format_float_scientific(x, precision=10, unique=False)
    ...
    with open(os.path.join(path, "user_mel_tables.c"), "wt") as f:
        ...
        if floatingPoint == "default":
            f.write(
                "const float32_t user_win[{}] = {};\n".format(len(hannWin), hannWin_str)
            )
            f.write(
                "const float32_t user_melFiltersLut[{}] = {};\n".format(
                    len(melFilterLut), melFilterLut_str
                )
            )
        else:
            f.write(
                "const PREPROC_FLOAT_T user_win[{}] = {};\n".format(
                    len(hannWin), hannWin_str
                )
            )
            f.write(
                "const PREPROC_FLOAT_T user_melFiltersLut[{}] = {};\n".format(
                    len(melFilterLut), melFilterLut_str
                )
            )
        f.write(
            "const uint32_t user_melFiltersStartIndices[{}] = {};\n".format(
                len(melFilterStartIndices), melFilterStartIndices_str
            )
        )
        f.write(
            "const uint32_t user_melFiltersStopIndices[{}] = {};\n".format(
...
def generate_mel_LUT_files(config):
    ...
    hannWin = _generate_hann_window_LUT(
        window=config.feature_extraction.window,
    ...
    if config.deployment.hardware_setup.serie == "STM32N6":
        ...
        _generate_LUTs_header_file(
            path, melFilterLut, melFilterStartIndices, melFilterStopIndices, hannWin
        )
    print("[INFO] : Done generating LUT header file")
    print("[INFO] : Generating LUT C file")
    if config.deployment.hardware_setup.serie == "STM32N6":
        _generate_LUTs_c_file(
            path,
            melFilterLut,
            melFilterStartIndices,
            melFilterStopIndices,
