...
def generate_ranked_vocabulary(vocabulary, audioset_ontology, output_file=None):
    ...
    fsd_vocab = pd.read_csv(vocabulary, header=None, names=["id", "name", "mids"])
...
def _remove_labels(labels, to_keep):
    ...
    labels = labels.split(",")
    kept = [label for label in labels if label in to_keep]
    labels = ",".join(kept)
    return labels
...
def filter_labels(
    ...
    intermediates = ranked_vocabulary[ranked_vocabulary["rank"] == "Intermediate"][
        "name"
    ].to_list()
...
def unsmear_labels(
    ground_truth, vocabulary, audioset_ontology, output_file=None, save=False
):
    """
    ...
    if only_keep_monolabel:
        ...
        if preferred_collapse_classes is None:
            if classes_to_keep is None:
                raise ValueError(
                    "If collapse_to_monolabel is true, \
