...
def _add_garbage_class_to_df(
    ...
    for ood_class in other_classes:
        sample = dataframe[dataframe["category"] == ood_class].sample(
            n=n_samples_per_other_class, random_state=seed, replace=True
        )
...
def _get_ds(
    ...
    if not set(used_classes).issubset(set(available_classes)):
        raise ValueError(
            f"""Some classes in class_names were not found
    ...
    if not used_classes:
        raise ValueError(
            "Did not receive any classes to use. Please check the class_names arg in your config file"
        )
    ...
    if use_garbage_class:
        if n_samples_per_garbage_class:
            ...
            num_other_class = len(
                set(df["category"].unique()).difference(set(used_classes))
            )
    ...
    string_lookup_layer = tf.keras.layers.StringLookup(
        vocabulary=sorted(
            list(vocab)
        ),  # Note we're sorting classes alphabetically here
        num_oov_indices=0,
        output_mode="one_hot",
    )
    ...
    if shuffle:
        if return_clip_labels:
            ...
            print(
                "[WARNING] Tried to shuffle a dataset which has associated clip labels. \n \
...
def _load_ESC_10(
    ...
    if not class_names:
        raise ValueError(
            "Argument class_names was not provided ! \
    ...
    if validation_csv_path is not None:
        ...
        train_esc_df, val_esc_df = train_test_split(
            esc_df,
            test_size=validation_split,
            random_state=seed,
            stratify=esc_df["category"],
        )

    else:
        # Use fold 5
        train_esc_df, val_esc_df = (
            esc_df[esc_df["fold"] != 5],
            esc_df[esc_df["fold"] == 5],
        )
...
def _prepare_fsd50k_csvs(
    ...
    print(
        "Successfully generated model zoo-compatible CSV for training set at {}".format(
            output_file
        )
    )
    ...
    print(
        "Successfully generated model zoo-compatible CSV for test set at {}".format(
            output_file
        )
    )
...
def _load_FSD50K(
    ...
    print(
        "[INFO] : The dataset.training_*, and dataset.validation_* args in your config file will be ignored."
    )
    if not class_names:
        raise ValueError(
            "Argument class_names was not provided ! \
    ...
    # Use the eval set
    print(
        "[INFO] : Loading validation dataset, using FSD50K's eval set as validation dataset."
    )
    val_ds, val_clip_labels = _get_ds(
        df=val_df,
...
def load_dataset(
    ...
    if dataset_name.lower() == "fsd50k":
        ...
        if dataset_name.lower() == "esc10":
            ...
            raise ValueError(
                "Dataset name must be one of the following : \n \
