...
def _tflite_ptq_quantizer(
    ...
    quantization_output_type: str = None,
    quantization_split: str = None,
    quantize_with_full_dataset: bool = False,
    ...
    def _representative_data_gen():
        ...
        if fake:
            ...
            if quantize_with_full_dataset:
                print(
                    "[INFO] : Quantizing by using the provided dataset fully, this will take a while."
                )
                ...
                for patches, _ in tqdm.tqdm(
                    quantization_ds, total=len(quantization_ds)
                ):
                ...
                print(
                    f"[INFO] : Quantizing by using {quantization_split * 100} % of the provided dataset..."
                )
                split_ds = quantization_ds.take(
                    int(len(quantization_ds) * float(quantization_split))
                )
...
def quantize(
    ...
    _, input_shape = get_model_name_and_its_input_shape(
        model_path=model_path, custom_objects=AED_CUSTOM_OBJECTS
    )
    ...
    if (
        cfg["quantization"]["quantizer"].lower() == "onnx_quantizer"
        and cfg["quantization"]["quantization_type"] == "PTQ"
    ):
        if quantization_ds:
            quant_split = (
                cfg.dataset.quantization_split
                if cfg.dataset.quantization_split
                else 1.0
            )
            print(
                f"[INFO] : Quantizing by using {quant_split * 100} % of the provided dataset..."
            )
            quantization_ds_size = len(quantization_ds)
            splited_ds = quantization_ds.take(
                int(quantization_ds_size * float(quant_split))
            )
            data, _ = tf_dataset_to_np_array(splited_ds, nchw=False)
        else:
            print("[INFO] : Quantizing by using fake dataset...")
            data = None
        ...
        if file_extension == ".h5":
            ...
            converted_model_path = os.path.join(
                output_dir, "converted_model", "converted_model.onnx"
            )
        ...
        if (
            ...
            if quantization_granularity == "per_tensor" and quantization_optimize:
                ...
                optimized_model_path = os.path.join(
                    output_dir, export_dir, "optimized_model.h5"
                )
            ...
            quantized_model_path = os.path.join(
                output_dir, export_dir, "quantized_model.tflite"
            )
            return quantized_model_path
        else:
            raise NotImplementedError(
                "Quantizer and quantization type not supported yet!"
            )
