...
def _block2_custom(x, filters, kernel_size=3, stride=1, conv_shortcut=False, name=None):
    ...
    x = layers.Conv2D(filters, kernel_size, strides=stride, name=name + "_2_conv")(x)
...
def _get_scratch_model(
    ...
    backbone = _MiniResNetV2(
        n_stacks=n_stacks, input_shape=input_shape, weights=None, pooling=pooling
    )
    add_flatten = pooling is None or pooling == "None"
    miniresnetv2 = add_head(
        backbone=backbone,
        n_classes=n_classes,
...
def _get_pretrained_model(
    ...
    # Load model
    if pooling == "avg":
        miniresnetv2 = tf.keras.models.load_model(
            Path(
...
def get_model(
    ...
    if not pretrained_weights:
        miniresnetv2 = _get_scratch_model(
            input_shape=input_shape,
            n_stacks=n_stacks,
            n_classes=n_classes,
            pooling=pooling,
        ...
        miniresnetv2 = _get_pretrained_model(
            n_stacks=n_stacks,
            n_classes=n_classes,
            pooling=pooling,
            use_garbage_class=use_garbage_class,
