...
def _check_parameters(embedding_size: int, patch_length: int, n_mels: int):
...
def _get_transfer_learning_model(
    n_classes: int = None,
    ...
    yamnet_backbone = tf.keras.models.load_model(
        Path(
...
def get_model(
    ...
    if pretrained_weights:
        yamnet = _get_transfer_learning_model(
            n_classes=n_classes,
        ...
        raise RuntimeError(
            "Yamnet is only available with pretrained weights. \n \
