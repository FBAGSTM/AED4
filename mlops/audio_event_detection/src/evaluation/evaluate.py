...
# ------------------------------------------------------------------------------------------
# NOTE: This function has been adapted for our AWS-GitHub architecture.
# In the original Model Zoo, the function does not return a report dictionary (e.g., sagemaker_report_dict).
# This return value was added to facilitate integration with AWS SageMaker pipelines,
# enabling automated metric reporting and conditional workflow steps.
def evaluate(cfg: DictConfig = None, eval_ds: tf.data.Dataset = None,
    ...
    q_patch_level_acc = None
    q_clip_level_acc = None
    ...
    if target in ["stedgeai_host", "stedgeai_n6"]:
        ...
        q_patch_level_acc, q_clip_level_acc = _evaluate_stairunner(input_samples=data,
                             patch_labels=labels,
                             clip_labels=clip_labels,
        ...
        try:
            ...
            if file_extension == '.tflite':
                ...
                q_patch_level_acc, q_clip_level_acc = _evaluate_tflite_quantized_model(cfg=cfg, 
            ...
            # Check if the model is a Keras model
                ...
                q_patch_level_acc, q_clip_level_acc = evaluate_h5_model(model_path=model_path,
                                eval_ds=eval_ds,
                                class_names=class_names,
                                clip_labels=clip_labels,
                                output_dir=output_dir,
                ...
                data, labels = tf_dataset_to_np_array(eval_ds, nchw=False)
                q_patch_level_acc, q_clip_level_acc = _evaluate_onnx_model_aed(input_samples=data,
                                        patch_labels=labels,
                                        clip_labels=clip_labels,
                                        input_model_path=model_path,
                                        class_labels=class_names,
